...
- 开发必须严格工程化推进，所有实现都要可跟踪、可归档、可审计、可约束
- 最终产物必须保持为通用型 Skill，不能把核心能力写死在单一宿主平台
- 所有实现都必须为“可自适应、可扩展、具备神经单元网格式激活传播能力的数字赛博生态圈产品”预留扩展空间

若某一步最小实现与以上约束冲突，应优先修改实现方案，而不是降低约束标准。
