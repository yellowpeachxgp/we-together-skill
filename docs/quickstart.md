...
> 适用于 we-together-skill v0.20.1。目标是在 5 分钟内完成一键安装，并跑起一个可对话的小社会图谱。
...
~/.we-together/venv
~/.we-together/data
~/.codex/skills/we-together*
~/.codex/config.toml
```

安装器不会安装 Python、git 或 Codex 本体；这些属于系统前置。
...
## 7. Codex skill family

一键安装已经自动安装 skill family 并注册 MCP。手动路径：
